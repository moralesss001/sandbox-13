...
def test_market_mode_gate_keeps_no_trade_as_analytics_only():
    ...
    assert result.gate_type == "analytics"
...
def test_candidate_carries_shadow_gate_metadata_and_production_decision():
    ...
    assert candidate.production_block_reasons == ["rsi_below_35"]
    assert candidate.raw["production_would_allow"] is False
    assert candidate.raw["shadow_gate_block_reasons"] == ["rsi_below_35"]
    market_gate = next(gate for gate in candidate.shadow_gates if gate["gate_name"] == "market_mode_15m_gate")
    assert market_gate["value"] == "NO_TRADE"
    assert market_gate["reason"] == "market_mode_15m_no_trade"


def test_market_mode_alone_does_not_change_production_decision():
    candidate = attach_shadow_gate_metadata(_candidate(rsi=50, raw={"market_mode_15m": "NO_TRADE"}))
